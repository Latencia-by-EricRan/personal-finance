...
**SSR + hydration + zoneless.** `app.config.ts` wires `provideClientHydration()`, `provideExperimentalZonelessChangeDetection()`, and `provideHttpClient(withFetch())`. `app.config.server.ts` adds `provideServerRendering()` plus `provideServerRoutesConfig(serverRoutes)` for the server bundle. `server.ts` uses the modern `AngularNodeAppEngine` (not the legacy `CommonEngine`) — this requires `angular.json`'s build options to set `"outputMode": "server"` alongside `"ssr": { "entry": "server.ts" }`, otherwise the build silently omits the app-engine-manifest wiring and the server throws at runtime. Per-route rendering mode is set in `src/app/app.routes.server.ts`: `login` is `RenderMode.Prerender` (public, static), everything else (`**`) is `RenderMode.Client` — the auth-guarded dashboard routes are intentionally excluded from SSR/prerender so `authGuard` never evaluates against `AuthService.isAuthenticated`'s server-seeded (always-false, since `localStorage` doesn't exist server-side) signal; they render fully client-side instead. Keep this in mind when writing components that touch browser-only APIs — guard them appropriately, and don't move a route to `RenderMode.Prerender`/`.Server` without checking whether it depends on client-only auth state.

**API base URL is environment-driven**, not hardcoded. Services inject the `API_BASE_URL` token (`core/tokens/api-base-url.token.ts`), which defaults to `environment.apiUrl` (swapped between `environment.ts`/`environment.development.ts` via `angular.json` `fileReplacements`) for browser/Karma. `app.config.server.ts` overrides it for SSR with `process.env['API_URL_SERVER']`, falling back to `http://localhost:3000` only if unset.
