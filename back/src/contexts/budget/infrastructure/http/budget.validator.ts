...
/**
 * Copied 1:1 from the legacy `modules/validators/budget.validator.ts`
 * (design PR3): route table/validation behavior stays byte-identical, only
 * the file's location changes to become `budget`'s inbound HTTP
 * infrastructure. See the PR3.1 characterization baseline
 * (`src/e2e/budget-http.characterization.e2e.test.ts`) for the pinned
