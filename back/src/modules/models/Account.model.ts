...
// consumers (recurring.service.ts, e2e suites, scripts/seed.ts) working
// without a double registration, which would throw `OverwriteModelError`.
// `Recurring.model.ts`'s `ref:'Account'` and `MovementModel.ts`'s
// `ref:'Account'` are string refs resolved by the preserved registration
// name, so they are unaffected by this shim too. (The layer-first
// `account.service.ts` was deleted in PR4 and no longer uses this shim.)
